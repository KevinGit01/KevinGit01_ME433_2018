...
unsigned char SPI1_IO(short ch) {
    SPI1BUF = ch;
...
void setVoltage(char channel, short voltage){
  unsigned char MSB, LSB = 0b00000000;
    ...
    SPI1_IO(MSB);
    SPI1_IO(LSB);      
    CS = 1;                        // raise the chip select line, ending communication                     // raise the chip select line, ending communication
}